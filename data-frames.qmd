...
There are useful base R functions to read data from external files or internet addresses, but we shall learn how to use their enhanced counterparts stored in the package `readr` which is one of the core packages of the tidyverse.

Usually data are stored either in excel files or in CSV files (comma separated variable). Let's start with the latter and suppose we want to store in R the content of the [**sheep** data set](https://github.com/acarpignani/R4DS/blob/main/datasets/sheep.csv) on the course repository. This contains the height (in cm) and the weight (in kg) of a sample of sheep. To do so, providing the tidyverse is loaded, it suffices to run the command
...
Let's load another data set, from our repository. This time we are interested in the data set containing information about the [density of worms](https://github.com/acarpignani/R4DS/blob/main/datasets/worms.csv) in some fields. Let's call this data frame `worms`.

...
Let us also add some transparency. This is done using the parameter `alpha` which is a number that goes from 0 (total transparency) to 1 (no transparency). For example, let's set `alpha = 0.7`.
...
I find the numbers on the y-axis to be really ugly and would rather having normal numbers there. Since densities are always numbers between 0 and 1, they are very clear in percentage. Let's use the function `scales::percent` to do so. Let's also change the labels on the axes.
